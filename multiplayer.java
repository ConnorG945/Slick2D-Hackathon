    ...
    public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
        ...
        if (win(garbage, inventory)) {
            ...
            if (inShop) {
                ...
                //2nd Player
                if (input.isKeyDown(Input.KEY_RIGHT) && murX < 800 - 16) {
                    murX += 4+boot;
                    mur = right;
                }
                if (input.isKeyDown(Input.KEY_LEFT) && murX > 0) {
                    murX -= 4 +boot;
                    mur = left;
                }
                if (input.isKeyDown(Input.KEY_UP) && murY > 150) {
                    murY -= 4+boot;
                    mur = back;
                }
                if (input.isKeyDown(Input.KEY_DOWN) && murY < 360) {
                    murY += 4+boot;
                    mur = front;
                }
                //1st Player
                ...
                //2nd Player
                if (!hasBackpack && currency >= 7 && murX > backpack.getX() - 10 && murX < backpack.getX() + 28 && murY > backpack.getY() - 26 && murY < backpack.getY() + 20) {
                    hasBackpack = true;
                    ups[items] = backpack.getG();
                    items++;
                    currency-=7;
                }
                if (!hasNet && currency >= 6 && murX > net.getX() - 10 && murX < net.getX() + 28 && murY > net.getY() - 26 && murY < net.getY() + 20) {
                    hasNet = true;
                    ups[items] = net.getG();
                    items++;
                    currency-=6;
                }
                if (!hasBoots && currency >= 5 && murX > boots.getX() - 10 && murX < boots.getX() + 28 && murY > boots.getY() - 26 && murY < boots.getY() + 20) {
                    hasBoots = true;
                    ups[items] = boots.getG();
                    items++;
                    boot=2;
                ...
                if (!hasClock && currency >= 3 && murX > clock.getX() - 10 && murX < clock.getX() + 28 && murY > clock.getY() - 26 && murY < clock.getY() + 20) {
                    hasClock = true;
                    ups[items] = clock.getG();
                    items++;
                    currency-=3;
       ...
       */
            if (runningTime < 30000 + extraTime) {
                ...
                if (input.isKeyDown(Input.KEY_UP) && murY > 220) {
                    murY -= speed;
                    mur = back;
                }
                if (input.isKeyDown(Input.KEY_DOWN) && murY < 500 - 32) {
                    murY += speed;
                    mur = front;
                }
                if (hasNet) {
       ...
       */
                for (int i = 0; i < garbage.length; i++) {
                    if (inventory < maxInv) {
                        ...
                        if(hasNet&&murX > garbage[i].getX() - 15 && murX < garbage[i].getX() + 33 && murY > garbage[i].getY() - 26 && murY < garbage[i].getY() + 20)
                        {
                            garbage[i].setX(1000);
                            inventory++;
                        }
                        else
                        if (murX > garbage[i].getX() - 10 && murX < garbage[i].getX() + 28 && murY > garbage[i].getY() - 26 && murY < garbage[i].getY() + 20) {
                            garbage[i].setX(1000);
                            inventory++;
                        }
                    }
                }
                //Collision detection for clocks. If a clock is collected, 5 seconds will be added to the time.
                for (int i = 0; i < clocks.length; i++) {
                    if (salX > clocks[i].getX() - 10 && salX < clocks[i].getX() + 28 && salY > clocks[i].getY() - 26 && salY < clocks[i].getY() + 20) {
                        clocks[i].setX(900);
                        runningTime -= 5000;
                    }
                    if (murX > clocks[i].getX() - 10 && murX < clocks[i].getX() + 28 && murY > clocks[i].getY() - 26 && murY < clocks[i].getY() + 20) {
                        clocks[i].setX(900);
                        runningTime -= 5000;
                    }
                }
                ...
                for (int i = 0; i < coins.length; i++) {
                    ...
                    if (murX > coins[i].getX() - 10 && murX < coins[i].getX() + 28 && murY > coins[i].getY() - 26 && murY < coins[i].getY() + 20) {
                        coins[i].setX(900);
                        currency++;
                    }
                }
                ...
                //2nd Player
                if (murX > trashCan.getX() - 14 && murX < trashCan.getX() + 30 && murY > trashCan.getY() - 5 && murY < trashCan.getY() + 30) {
                    score += inventory;
                    inventory = 0;
                }
            }
